...
### Converter release: Crate version 0.6.0 (raptrix-cim-arrow) / 0.6.0 (raptrix-cim-rs) | Arrow schema v0.13.0
...
- **Timestamps**: `timestamp_utc`, `snapshot_timestamp_utc`, `sal_enhancement_timestamp`, `scenario_context.created_timestamp_utc` → Arrow `Timestamp(us, UTC)`.
- **`buses.type`**: Dictionary tokens `PQ` | `PV` | `Slack` (not Int8 1/2/3).
- **`buses_solved.bus_type_solved`**: same dictionary tokens (nullable).
- **`generators.controlled_bus_id`**: nullable; **null = local regulation** (no `0` sentinel).
- **Optional `mrid`** on `loads`, `fixed_shunts`, `switched_shunts`.
- **`dynamics_models.classical_params`**: nullable struct `{H, D, xd_prime, mbase_mva}`.
- **`computational_load_profiles`**: large-load candidate columns (`priority`, `trip_study_percentiles` as **0–100 percentage points**, `common_mode_group`, facility fields, ride-through maps, etc.). Null/empty percentiles mean no auto-generated percentiles from the case file.
- File metadata `rpf.identity.model = hybrid_solver_flat_v1` on CIM exports.
...
- **Nullable `mrid` column** on `branches`, `generators`, `transformers_2w`, and `transformers_3w` for stable CIM-compatible equipment identifiers.
- **Schema metadata key `rpf.mrid_support = v1`** for introspection by downstream readers.
- **CIM importer population**: `ACLineSegment`, `SynchronousMachine`, and `PowerTransformer` mRIDs are written on export; star-expanded 3W legs receive deterministic `{mrid}_H` / `_M` / `_L` suffixes.

### Changed

- **`SUPPORTED_RPF_VERSIONS`** now accepts **`v0.12.2`** / **`0.12.2`** and retains **`v0.12.1`** / **`0.12.1`** for backward reads.
- `RPF_VERSION` / `SCHEMA_VERSION` / `BRANDING` bumped to v0.12.2.
